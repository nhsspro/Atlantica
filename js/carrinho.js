...
// Definição de uma variável de exemplo para códigos promocionais válidos
const validPromoCodes = {
    "ATLANTICA10": 0.10, // 10% de desconto
    "ATLANTICA20": 0.20, // 20% de desconto
    "FRETEGRATIS": 0.00 // Frete grátis (aplicando desconto no valor total)
};

// Função para aplicar o código promocional
function applyPromoCode() {
    const promoCodeInput = document.getElementById('promo-code');
    const promoCode = promoCodeInput.value.trim().toUpperCase(); // Obtém e normaliza o código

    // Verifica se o código é válido
    if (validPromoCodes[promoCode]) {
        const discount = validPromoCodes[promoCode]; // Obtém o valor do desconto
        ...
        document.getElementById('final-price').innerText = `R$ ${finalPrice.toFixed(2)}`;
        document.getElementById('total-price').innerText = `R$ ${finalPrice.toFixed(2)}`;

        // Limpa o campo de código promocional
        promoCodeInput.value = '';

        // Opcional: Pode-se exibir uma mensagem de sucesso
        alert(`Código promocional aplicado com sucesso! Desconto de ${discount * 100}%`);
    } else {
        // Se o código não for válido, exibe um alerta
